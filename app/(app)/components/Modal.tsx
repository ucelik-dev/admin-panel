...
const Modal = ({handleDelete, setModal}:Props) => {
  return (<>
    <div id="deleteModal" aria-hidden="true" className="bg-gray-200 bg-opacity-50 h-screen flex overflow-y-auto overflow-x-hidden fixed top-0 right-50 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
        <div className="relative p-4 w-full max-w-md h-full md:h-auto">
        
            <div className="relative p-4 text-center bg-white rounded-lg shadow sm:p-5">
                <button onClick={() => setModal(false)} type="button" className="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteModal">
                    <IoMdClose size="20"/>
